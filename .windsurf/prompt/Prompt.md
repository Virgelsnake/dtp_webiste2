...
### 3.5. Managing the Python Environment

Your Python environment must follow a strict setup protocol to ensure compatibility and avoid conflicts across tools, terminals, and IDEs.

*   **Use `pyenv` for version management:** Python must be installed and managed using `pyenv`. Do **not** use Homebrew, system Python, Conda, or Poetry unless explicitly approved.

    > **Approved Version:** Python **3.11.9**  
    > To verify your active version, run:
    > ```bash
    > python --version
    > ```
    > You should see `Python 3.11.9`. If not, run:
    > ```bash
    > pyenv global 3.11.9
    > ```

*   **Use `virtualenv` for project isolation:**
    > **THIS IS CRITICAL:** Every project must use a dedicated virtual environment created with `virtualenv`. This prevents global package conflicts and supports reproducible builds.
    ...
    > ✅ **Remember:** Always activate the virtual environment before working on your project to ensure you're using the correct Python version (`3.11.9`) and all the project-specific dependencies.

*   **Do not use unauthorised Python environments:**
    Avoid the following unless specifically instructed:
    * `python -m venv`
    * Conda
    * Poetry
...
*   **Set the interpreter manually in your IDE:**
    > **THIS IS CRITICAL:** You must **manually select** the correct Python interpreter in your IDE. Do **not** allow the IDE to auto-detect system or Homebrew versions.

    Acceptable interpreter paths:
    * Global `pyenv` version:  
      `/Users/steveshearman/.pyenv/versions/3.11.9/bin/python`
    * Project virtualenv:  
      `/path/to/project/venv/bin/python`

*   **Test the environment after activation:**  
    To confirm your environment is active and correct, run:
    ...
    You should see the virtualenv path and `Python 3.11.9`.
...
**enforce_modern_ui_design**
*Description:* Enforces a modern, minimal UI aesthetic across all generated or modified interface components. Use this tool whenever a UI element is created or updated. Automatically triggers a screenshot post-render and validates visual alignment with the provided design reference. Ensures compliance with contemporary design standards (e.g. Apple-style minimalism).
`<enforce_modern_ui_design> {"$schema":"https://json-schema.org/draft/2020-12/schema","properties":{"UiComponentName":{"type":"string","description":"The name or identifier of the UI component being built or modified."},"DesignReference":{"type":"string","description":"Optional. A short description or URL reference to the intended design style or inspiration (e.g. 'Apple calendar', 'modern task board UI')."},"CaptureAfterRender":{"type":"boolean","description":"If true, automatically trigger a screenshot after the UI component is rendered or updated."},"ValidateScreenshotDesign":{"type":"boolean","description":"If true, perform a visual check to confirm that the rendered UI aligns with the described or expected changes."}},"additionalProperties":false,"type":"object","required":["UiComponentName","CaptureAfterRender","ValidateScreenshotDesign"]} </enforce_modern_ui_design>`
