...
| C3 | [X] Add a privacy policy page | All | Required for GDPR compliance and professional credibility. | **Done** |
...
| D3 | [X] Interactive Google Maps embed instead of static map placeholder | Contact | Reduces friction for visitors trying to find the office. | **Done** |
...
| S5 | [X] Timeline connector pulse animation | GitHub Copilot | On the process timeline, a mint-coloured pulse travels down the left border as the user scrolls through steps — like data flowing through a pipeline. Reinforces the "technology" brand. CSS `@keyframes` only. | **Done** — DevelopmentJourney component has animated timeline with pulse effects |

### Visual & Layout

| # | Idea | Inspiration | Description | Status |
|---|------|-------------|-------------|--------|
| S6 | [X] Gradient mesh hero backgrounds | Vercel, Resend | Replace the static grid background on hero sections with a subtle animated gradient mesh (noise-textured, slow-moving colour blobs). Can be done with CSS `radial-gradient` layers and a `@keyframes` animation cycling `background-position`. | **Done** |
| S7 | [X] "Bento grid" layout for benefits section | Apple, Linear | Replace the uniform 3-column benefits grid with a bento layout where key items span 2 columns. The most important benefit gets visual priority. More editorial, less template. | **Done** — Simplified to uniform 3-column grid for cognitive fluency |
| S8 | [ ] Monochrome illustration system | Resend, Cal.com | Commission or create simple monochrome line illustrations for each use case / service area. Displayed at reduced opacity over surface cards. Adds visual interest without photographs (which would feel generic). | **Rejected** — Prototypes added noise without clear benefit |
| S9 | [X] Split-screen hero for Services page | Stripe | Left side: headline + description. Right side: an abstract visualisation of the 5-stage process (connected nodes, flowing data). Animated on load. Much more memorable than text-only. | **Done** — Animated 5-node process pipeline with flowing data particles |
| S10 | [X] "Command palette" site search | Raycast, Linear | Press `Cmd+K` to open a search overlay that filters pages, FAQ questions, and case studies. Minimal implementation with fuzzy search. Makes the site feel like a product, not a brochure. | **Done** — Cmd+K fuzzy search across pages, FAQ, case studies, use cases |

### Content & Trust

| # | Idea | Inspiration | Description | Status |
|---|------|-------------|-------------|--------|
| S11 | [X] "Results ticker" in the hero stat bar | Stripe Atlas | Instead of static stats, show a subtle count-up animation when the stat bar enters the viewport. "100%" counts from 0. Adds dynamism and draws the eye. | **Done** — Stats count up from 0 with 1500ms ease-out animation, 200ms stagger |
| S12 | [X] Case study mini-timeline | Vercel case studies | Each case study card includes a 3-4 step mini timeline showing the actual project journey (weeks/stages). Makes the 5-stage process tangible with real data. | **Done** — Case studies have detailed challenge/solution/impact flow |
